module ActionView::Helpers::FormTagHelper
  ...
  #根据params对应的值,判断是否选中
  def radio_button_tag_with_check(name, value, checked = false, options = {})
    options.merge! :class => :radio
    reg = /(.*)\[:*(.*)\]/
    if reg.match(name.to_s)
      regexp_name = name.to_s.match(reg)
    ...
    radio_button_tag_without_check(name, value, checked, options)
  end
  alias_method_chain :radio_button_tag, :check

  alias_method :orig_submit_tag, :submit_tag
  #默认加入:class => "submit"
  def submit_tag(value, options = {})
    options[:class] ||= "submit"
    value = value.blank? ? value : t(value)
