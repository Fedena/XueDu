class ReminderController < ApplicationController
  ...
  def create_reminder
    ...
    if request.post?
      unless params[:reminder][:body] == "" or params[:recipients] == ""
        ...
        flash[:notice]="<b>错误:</b>请在创建消息时填写必填项."
        redirect_to :controller=>"reminder", :action=>"create_reminder"
      end
    end
  end
  ...
  def delete_reminder_by_sender
    ...
    flash[:notice] = m('reminder.delete.success')
  ...
  def send_reminder
    unless params[:create_reminder][:message] == "" or params[:create_reminder][:to] == ""
      ...
      render(:update) do |page|
        page.replace_html 'error-msg', :text=> '<p class="flash-msg">你的消息已成功发送.</p>'
      end
    else
      render(:update) do |page|
        page.replace_html 'error-msg', :text=> '<p class="flash-msg">请同时填写课目和内容.</p>'
      end
    end
  end
end
